...
const uiController = (function() {
  ...
  const createOptionList = () => {
    removeOptionList();
    dataController.projContainer.reduce((opt, proj)=>{
  ...
  const setPrioritySelectedIndex = (id) =>{
    ...
    let ind = Array.from(elements.constants.selectPriority.options)
      .findIndex(opt => { return opt.value === prior; });
    if (ind >= 0) {
  ...
  elements.constants.iconChevron.addEventListener('click',
    () => elements.showProjects.call(elements));
  ...
  elements.constants.taskForm.addEventListener('submit', function (event) {
    if (!elements.constants.inputTitle.validity.valid
        || !elements.constants.inputDueDate.validity.valid) {
      showError();
      event.preventDefault();
    }
  });

  document.querySelector('#dueDate').addEventListener('input', function () {
